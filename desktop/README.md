...
The 2026-09-04 preview passed the Electron smoke directly from its read-only mounted Mac DMG with `PATH=/usr/bin:/bin`, as well as deep/strict ad-hoc signature and disk-image integrity checks. The Windows installer and its inner payload passed archive tests; recursive runtime checks verified target binaries, resource hashes and internal-only symlinks. No Windows execution or clean-machine result is claimed.
...
- Complete updater behavior, publisher signing/notarization and clean-machine installer testing before public distribution. Startup progress, resource checks and unsigned installer generation exist. Guide videos remain deferred until desktop UI behavior settles.
