...
const destination = path.join(root, 'desktop/artifacts/installers/Annotate-0.2.2-desktop.1-source.tar.gz');
...
for (const file of candidates) {
  if (!/^(webapp\/|sidecar\/|desktop\/|scripts\/|\.github\/|package(-lock)?\.json$|LICENSE$|README\.md$|technical_document\.md$)/.test(file)) continue;
