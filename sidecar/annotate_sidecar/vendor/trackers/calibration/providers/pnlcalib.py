...
# Modified by Annotate contributors for application integration.
# ------------------------------------------------------------------------
...
from annotate_sidecar.vendor.trackers.calibration.base import PitchCalibrator
from annotate_sidecar.vendor.trackers.calibration.projection import invert_homography
from annotate_sidecar.vendor.trackers.calibration.smoothing import fill_calibration_gaps
from annotate_sidecar.vendor.trackers.calibration.types import CalibrationFrame, PitchDimensions
...
class PnLCalibProvider(PitchCalibrator):
    ...
    def availability_hint(self) -> str | None:
        ...
        if importlib.util.find_spec("shapely") is None:
            return (
                "PnLCalib needs the `shapely` package in the current "
                "environment. Install it with `pip install shapely>=2.0.0`."
            )
        missing_weights = []
