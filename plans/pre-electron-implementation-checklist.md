...
This checklist implements the direction in [Desktop Application Direction](desktop-application-direction.md). Completed steps must preserve project compatibility and browser workflows. There is no project-schema migration, UI redesign, browser version bump or tag change. Desktop packages use the separate preview version `0.2.2-desktop.1`. Electron 44.2.0, esbuild 0.28.2 and electron-builder 26.15.3 are pinned root development dependencies. The sidecar dependency lock is unchanged; packages bundle Python 3.12.14.
...
- [ ] Add signing/notarization and platform installer/update handling before public desktop distribution. App Store distribution remains out of scope.
