...
This checklist implements the direction in [Desktop Application Direction](desktop-application-direction.md). Completed steps must preserve project compatibility and browser workflows. There is no project-schema migration, UI redesign, browser version bump or movement of existing tags. The refreshed desktop packages use version `0.2.2-desktop.2`; both its Mac DMG and Windows x64 installer are distributed under the existing `v0.2.2-desktop.2` tag. GitHub's prerelease flag is off and Latest is selected for homepage visibility; the builds remain unsigned evaluation previews. Superseded desktop.1 artifacts remain local. Electron 44.2.0, esbuild 0.28.2 and electron-builder 26.15.3 are pinned root development dependencies. The sidecar dependency lock is unchanged; packages bundle Python 3.12.14.
...
- [ ] Add signing/notarization and platform installer/update handling before claiming stable desktop support. The unsigned downloads remain evaluation builds regardless of GitHub's Latest label. App Store distribution remains out of scope.
...
2026-09-19, Windows distribution and install-document cleanup: attached the existing Windows desktop.2 installer to the same release and verified its uploaded checksum against the locally tested archive. GitHub does not allow prereleases to be Latest; at the user's request, the release is reclassified and marked Latest without removing evaluation/signing/testing caveats or moving the tag. The root browser installer and launchers moved to `legacy/browser-install/`, with archived install instructions and path resolution for both old release checkouts and the new layout. Browser development and service scripts remain active, with setup documented in `docs/development.md`. README, offline/in-app guides and desktop release documentation now point to Mac and Windows downloads. All four isolated legacy-launcher checks, shell syntax, strict lint, TypeScript and local documentation-link checks pass. A real Chromium check confirms guide navigation, desktop installation text and no horizontal overflow at 390 px. This changes documentation and release metadata, not either installed binary or Windows execution certification; the original source snapshot remains attached unchanged.
